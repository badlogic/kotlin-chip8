...
class Disassembler(): Decoder {
    ...
    /** all the '+' used here are actually StringBuilder.plus()
     * I'm not sure how string templates, i.e. "op: $opCode" work in terms
     * of object allocation, but I bet it's the same as String + String.
     * I think this is better since everything goes through the builder
     */
    override fun before(opCode: Int, address: Int) {
        builder + "addr: 0x" + address.toHex() + ", op: 0x" + opCode.toHex() + ": "
    }
    override fun unknown(opCode: Int) {
        builder + "Unknown opcode"
    }
    /** dropping the dots and parens for single arg methods is totally just a style preference
     * I think sometimes it's more readable */
    override fun clear() {
        builder line "clear"
    }
    override fun ret() {
